...
# Initial instance of QAModel
qa_model = QAModel(model_name, read_tale_from_file(tale_path), questions)

@app.route('/', methods=['GET', 'POST'])
def index():
    global qa_model  # Use the global variable

    data = {
        'titulo': 'LAB3 - ML2',
        'bienvenida': 'Lab III - Advanced Topics',
        'answer': None,
    ...
    if request.method == 'POST':
        ...
        qa_model = QAModel(selected_model, read_tale_from_file(tale_path), questions)

        if "sentence-transformers" in selected_model.lower():
            # Use SentenceTransformer for MiniLM or other embeddings
